...
def main():
    ...
    url = "https://careers.bcg.com/search-results?rk=page-targeted-jobs-page324-prod-n6mBQm"
    ...
    for i in range(1, pageNum):
        url = f"https://careers.bcg.com/search-results?from={i * 10}&s=1&rk=page-targeted-jobs-page324-prod-n6mBQm"
        jobData = response.split('"eagerLoadRefineSearch":')[
            1].split(',"flashParams":')[0]
        jobData = json.loads(jobData)
        jobs = jobs + jobData['data']['jobs']

    company = Company.objects.get(name=CompanyChoices.BCG)
